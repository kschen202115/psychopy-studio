#!/usr/bin/env python3
"""Transport-agnostic PsychoPy backend core for PsychoPy Studio Web.

This module holds all Builder semantics with NO transport dependency: it
validates request payloads, isolates browser-supplied paths, serializes
JSON-safe responses, and delegates to PsychoPy's official ``psychopy.experiment``
and compiler APIs. The single entry point is ``handle_command(command, args,
kwargs)``.

It is loaded both by the Pyodide Web Worker (runs entirely in-browser, no
server) and by the optional ``official_backend.py`` WebSocket dev server.
"""
